...
# TODO: Put your implemententation for EncryptMessage here.	
	...
	# $s1: number of chars in msg
	# $s2: shifted address
	# $t0: single char
	# $t1: boolean
	
	# keep count of the number of chars encrypted
	add $s0 $zero $zero
	
	# store the initial address into $s2
	add $s2 $zero $a0
...
Encryption:
	...
	# check if char is within bound
	slti $t1 $t0 91
	beq $t1 $zero ModulusOver
	slti $t1 $t0 65
	bne $t1 $zero ModulusUnder
	j Store
...
Finish:	
	# update number of chars left and address
	addi $s0 $s0 1
	addi $s2 $s2 1
	
	# check if there is chars left to encrypt
